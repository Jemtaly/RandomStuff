...
int main(int argc, char **argv, char **env) {
	...
	for (Vboolfuck vbfo; vbfo.eval(), GetKeyState(VK_ESCAPE) >= 0; Sleep(1), vbfo.clk = !vbfo.clk) {
		...
		mvprintf( 0,  0, "lqqqqqqqqqqqqqq CODE qqqqqqqqqqqqqqw");
		for (int i = 0; i < 8; i++) {
			mvprintf(i +  1,  0, "x ");
			for (int j = 0; j < 32; j++)
		...
		mvprintf( 9,  0, "tqqqqqqqqqqqqqq DATA qqqqqqqqqqqqqqu");
		for (int i = 0; i < 8; i++) {
			mvprintf(i + 10,  0, "x ");
			for (int j = 0; j < 32; j++)
		...
		mvprintf(12, 36, "x CTL x q %d q x", vbfo.ctl & 1);
		...
		mvprintf(19 + vbfo.top / 16, vbfo.top % 16 * 3 + 2, "%02X", vbfo.stk[vbfo.top]);
		attrno(UNDERLN);
		attron(REVERSE);
		switch (vbfo.blk) {
